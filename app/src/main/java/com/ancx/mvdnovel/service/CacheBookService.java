...
public class CacheBookService extends Service implements OnBookDirectoryListener, OnReadBookListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        numOfCached = 0;
        // 创建小说文件夹
        boolean modelReadBookDir = modelReadBook.createDir(book.get_id());
        if (!modelReadBookDir) {
    ...
    public void setBookBody(ChapterBody chapterBody) {
        modelReadBook.cacheText(book.get_id(), chapters.get(numOfCached).getTitle(), chapterBody.getBody(), sharedPreferences, chapters.get(numOfCached).getLink());
        continueCache();
    }
